...
def fluxo1():
    ...
    prefixo_resumo = config.get('arquivos','prefixo_resumo')
    nome_resumo_analitico = prefixo_resumo + today(string=True).replace('-','') + '.csv'
    ...
    subj_monitoracao = config.get('email','subj_monitoracao')
    html_monitoracao = config.get('email','html_monitoracao')
    monit_body = open(html_monitoracao, 'r')
    html2 = monit_body.readlines()
    html2 = ''.join(html2)
    ...
    # atestados_empregados.to_csv('teste.csv')
...
#=== NODE 03 ==========================================================================#
    ...
    try:
        ...
        if email_test:
            ...
            email(subj_monitoracao, html2, email_copy, nome_resumo_analitico)
    except Exception as e:
        log.logger.exception('Falha ao gerar arquivo CSV (feche o mesmo caso esteja aberto). Erro: ' + str(e))
...
#=== PRINTS ==========================================================================#
    ...
    print('[RESUMO ANALITICO] Arquivo: iEVO_atestados_20190425.xlsx')
    print('')
    print('-' * 112)
    print('[NODO #1] Atestados com CID [C; D; F; M; S; T]')
    print('[Resultado] # Funcionários enquadrados neste critério: ' +str(email_ok_node_01+email_er_node_01).zfill(3))
    ...
    print('[Resultado] # Funcionários enquadrados neste critério: ' +str(email_ok_node_02+email_er_node_02).zfill(3))
    ...
    print('[Resultado] # Funcionários enquadrados neste critério: ' +str(email_ok_node_03+email_er_node_03).zfill(3))
